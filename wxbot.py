...
class WXBot:
    ...
    def get_contact(self):
        ...
        #貌似这里添加的是保存到通讯录的组成员的信息,如果没添加,则不会进入,会在接收到群消息的时候收到组成员信息.
    ...
    #获取临时聊天室信息
    def batch_get_temporary_group_members(self,chatroomid):
        """批量获取所有群聊成员信息"""
        url = self.base_uri + '/webwxbatchgetcontact?type=ex&r=%s&pass_ticket=%s' % (int(time.time()), self.pass_ticket)
        params = {
            'BaseRequest': self.base_request,
            "Count": len(self.group_list),
            "List": [{"UserName": chatroomid, "EncryChatRoomId": ""}]
        }
        r = self.session.post(url, data=json.dumps(params))
        r.encoding = 'utf-8'
        dic = json.loads(r.text)
        ...
        for group in dic['ContactList']:
            gid = group['UserName']
            members = group['MemberList']
            temporary_group_members[gid] = members
            encry_chat_room_id[gid] = group['EncryChatRoomId']
       # self.temporary_group_members = temporary_group_members
       # self.encry_chat_room_id_list = encry_chat_room_id
    ...
    def get_group_member_name(self, gid, uid):
        ...
        if gid in self.temporary_group_members:
            group = self.temporary_group_members[gid]
            print('uid:',uid)
            print('gid:',gid)
            #print_tree(self.temporary_group_members);
            if uid in group:
                member = group[uid]
                names = {}
                if 'RemarkName' in member and member['RemarkName']:
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if msg_type_id == 0:
            ...
            uid = uid[:-1]
            #先按照自己联系人备注名字,然后按照群用户修改的群聊名字,然后再是用户自己的名字
    ...
    def handle_msg(self, r):
        ...
        """
        #CURR
        #处理消息之前,r的ModContactCount>0有可能更新了联系人信息(比如微信群里非好友的昵称等)
        if r['ModContactCount'] > 0:
            with open(u'更新临时群成员信息.json', 'w') as f:
                    f.write(json.dumps(r['ModContactList']))
            for ModContact in r['ModContactList']:
                print(u'++++添加临时群成员数据++++' + ModContact['NickName'])
                for Member in ModContact['MemberList']:
                    if Member['UserName'] not in self.account_info:#临时会话的好友信息肯定不需要保存,只保存陌生人的
                        #用组的UserName(其实就是id)保存对应的组里的临时聊天陌生人的信息
                        #print_tree(ModContact)
                        if u'weixin机器人' == ModContact['NickName']:
                            print('group id ' + ModContact['UserName'])
                            print('user id ' + self.to_unicode(Member['UserName']))
                            print('user_name ' + self.to_unicode(Member['NickName']))
                        #print_tree(Member)
                        #下面这种操作对字典有问题
                        #self.temporary_group_members[ModContact['UserName']] = { Member['UserName'] : Member }
                        if not self.temporary_group_members.has_key(ModContact['UserName']):
                            self.temporary_group_members[ModContact['UserName']] = {}
                        self.temporary_group_members[ModContact['UserName']][ Member['UserName']] = Member;
            if self.DEBUG:
                with open('temporary_group_members.json', 'w') as f:
                    f.write(json.dumps(self.temporary_group_members))

        #自己发消息到群的时候FromUserName是自己,@开头的id,ToUserName是群聊,@@开头的id
        #收到别人群消息的时候FromUserName是群,@@开头开头的id,ToUserName是自己,#开头的id
