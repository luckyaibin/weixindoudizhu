...
message_is_card_filter = u'234567890jJqQkKaAwW大小 ,，.．!！;：:；\'\"'
#把牌字符转换成对应到的点数
CARD_STR_TO_POINT_DICT = {
        u'3':3,
        u'4':4,
        u'5':5,
        u'6':6,
...
def CardDictToStr(card_point_dict):
    cardstrunicode = u''
    for point in card_point_dict:
        count = card_point_dict[point]["count"]
        for i in range(0,count):
            cardstrunicode += CARD_POINT_TO_STR_DICT[point]
    #cardstrunicode.sort()
    return  cardstrunicode

def CardListToCardDict(card_point_list):
    Len=len(card_point_list)
    card_point_dict={}
    for i in range(0,Len):
        card_point=card_point_list[i]
        dict_item=None
        if not card_point_dict.has_key(card_point):
            card_point_dict[card_point]={'count':0}
        dict_item = card_point_dict[card_point]
        dict_item['count'] += 1
    return card_point_dict

#检查扑克牌是不是足够
def CheckCardEnough(card_point_dict,card_point_dict_minus):
    for point in card_point_dict_minus:
        if not card_point_dict.has_key(point):#没有对应的牌点数
            return False
        else:
            count_own  = card_point_dict[point]['count']
            count_minus = card_point_dict_minus[point]['count']
            if count_own < count_minus:
                return False
    return True

#从扑克牌里减去一些牌
def MinusCard(card_point_dict,card_point_dict_minus):
    for point in card_point_dict_minus:
        count_own  = card_point_dict[point]['count']
        count_minus = card_point_dict_minus[point]['count']
        left_count = 0
        left_count = count_own - count_minus
        if left_count < 0:
            left_count = 0
        card_point_dict[point]['count'] = left_count


def PlusCard(card_point_dict,card_point_dict_plus):
    for point in card_point_dict_plus:
        if not card_point_dict.has_key(point):#没有对应的牌点数
            card_point_dict={'count':0}
        card_point_dict[point]['count'] += card_point_dict_plus[point]['count']
...
def ShuffleCard():
    ...
    for i in range(0,1):
        ...
        player2.append(c)
        del card_54[rand_index]
    cards_dict0 = CardListToCardDict(player0)
    cards_dict1 = CardListToCardDict(player1)
    cards_dict2 = CardListToCardDict(player2)
    cards_dict_left = CardListToCardDict(card_54)

    return cards_dict0,cards_dict1,cards_dict2,cards_dict_left


#10,和0都表示10
#单独的1是无效的输入
def InputIsCardString(msg):
    global i
    for i in range(len(msg)):
        c = msg[i]
        if message_is_card_filter.count(c)==0:#在出牌的字符串里没找到,那么有不属于扑克牌的字符,说明是其他输入
            return False
        '''
...
def CardStrToCardDict(card_string):
    i=0
    ...
    while i < length:
        c=card_string[i]
        if c in (CARD_STR_TO_POINT_DICT):
           if c==u'j':
               c=u'J'
           elif c==u'q':
               c=u'Q'
    ...
    cards={}
    for c in new:
        point=CARD_STR_TO_POINT_DICT[c]
...
if __name__ == '__main__':
    p0,p1,p2,left = ShuffleCard()
    print(p0,CardDictToStr(p0))
    print(p1,CardDictToStr(p1))
    print(p2,CardDictToStr(p2))
    print(left,CardDictToStr(left))
    ...
    for i in range(Len):
        ...
        if input_is_card_string(c,message_is_card_filter):
            c=CardStrToCardDict(c)
            print(get_card_type(c))
    '''
    print('-----------')
    cards_str1 = '8888'
    cards_str2 = '888889'
    print(InputIsCardString(cards_str1))
    print(InputIsCardString(cards_str2))
    cards1 = CardStrToCardDict(cards_str1)
    cards2 = CardStrToCardDict(cards_str2)
